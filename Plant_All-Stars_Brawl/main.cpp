...
Atlas atlas_peashooter_attack_left; // 婉逗射手朝向左的攻击动画图集
Atlas atlas_peashooter_attack_right; // 婉逗射手朝向右的攻击动画图集
...
Atlas atlas_sunflower_attack_left; // 龙日葵朝向左的攻击动画图集
Atlas atlas_sunflower_attack_right; // 龙日葵朝向右的攻击动画图集
...
void load_game_resources()
{
	...
	mciSendString(_T("open resources/ui_confirm2.mp3 alias ui_confirm2"), NULL, 0, NULL);
	mciSendString(_T("open resources/ui_switch3.mp3 alias ui_switch"), NULL, 0, NULL);
...
int main()
{
	...
	while (true)
	{
		...
		// 计算上一次调用on_update和这一次调用的时间间隔
		static DWORD last_tick_time = GetTickCount();		// 静态变量last_tick_time只在第一次初始化时赋值
		DWORD current_tick_time = GetTickCount();
		int delta_tick = current_tick_time - last_tick_time;
